...
import { editContentSchema, EditContentFormValues } from '@/schemas';
...
export const EditContentSheet = forwardRef<BottomSheetModal>((_, ref) => {
  const { editContentItem } = useContentStore();
  const { tags } = useTagsStore();
  const editingItem = useUiStore((s) => s.editingItem);

  const { control, handleSubmit, watch, setValue, reset, formState: { isSubmitting } } = useForm<EditContentFormValues>({
  ...
  useEffect(() => {
    if (editingItem) {
      reset({
        ...
        tags: editingItem.tags.map((t) => t._id),
      });
    }
  }, [editingItem, reset]);
  ...
  const onSubmit = async (data: EditContentFormValues) => {
    ...
    try {
      await editContentItem(editingItem._id, data);
      handleClose();
    } catch (err: any) {
...
EditContentSheet.displayName = 'EditContentSheet';
